...
{
    inline PFN_vkSetDebugUtilsObjectNameEXT fpVkSetDebugUtilsObjectNameEXT;

    void LoadDebugFunctions()
    {
        fpVkSetDebugUtilsObjectNameEXT = (PFN_vkSetDebugUtilsObjectNameEXT)(vkGetInstanceProcAddr(VulkanContext::GetVkInstance(), "vkSetDebugUtilsObjectNameEXT"));
        if (fpVkSetDebugUtilsObjectNameEXT == nullptr)
            fpVkSetDebugUtilsObjectNameEXT = [](VkDevice device, const VkDebugUtilsObjectNameInfoEXT* pNameInfo) { return VK_SUCCESS; };
    }
    ...
    void SetDebugName(VkObjectType objectType, const void* objectHandle, const std::string& name)
    {
        ...
        fpVkSetDebugUtilsObjectNameEXT(VulkanDevice::Get().GetDevice(), &info);
    }
    ...
    void TransitionImageLayout(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout)
    {
        ...
        {
            PX_CORE_ASSERT(false, "Unsupported image layout transition.");
            barrier.srcAccessMask = 0;
    ...
    void CreateImage(VkImage& outImage, uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usageFlags, VkMemoryPropertyFlags propFlags, VkDeviceMemory& outMemory, const std::string& debugName)
    {
        ...
        VulkanUtils::SetDebugName(VK_OBJECT_TYPE_IMAGE, outImage, debugName);
        ...
        VK_CHECK_RESULT(vkAllocateMemory(device, &allocInfo, nullptr, &outMemory));
        VK_CHECK_RESULT(vkBindImageMemory(device, outImage, outMemory, 0));
    }
    ...
    void CreateImageView(const VkImage& image, VkImageView& outImageView, VkImageViewType viewType, VkFormat format, VkImageAspectFlagBits aspectFlag, const std::string& debugName)
    {
        ...
        VK_CHECK_RESULT(vkCreateImageView(VulkanDevice::Get().GetDevice(), &viewInfo, nullptr, &outImageView));
        VulkanUtils::SetDebugName(VK_OBJECT_TYPE_IMAGE_VIEW, outImageView, debugName);
    }
}